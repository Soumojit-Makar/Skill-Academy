...
<div class="ftr">Automated notification from your Academy CRM.</div>
</div></body></html>`;
...
export const sendStudentAck = async (enquiry, aiSummary) => {
  const transporter = await getTransporter();
  const bodyText = aiSummary?.studentMailBody ||
    `<p>Thank you for reaching out to us, ${enquiry.name}. We have received your enquiry regarding <strong>${enquiry.courseName || 'our courses'}</strong>.</p>
